...
def generate_summary(game_info, moves):
    ...
    try:
        ...
        summary_35 = response_35.choices[0].message.content
        ...
        summary_4 = response_4.choices[0].message.content

        return summary_35, summary_4
    except OpenAIError as e:
        if "Rate limit" in str(e):
            st.error("API rate limit exceeded. Please try again in a moment.")
        elif "Invalid authentication" in str(e):
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if st.button("Generate Summary"):
            with st.spinner("Generating summary..."):
                moves_uci = " ".join(move.uci() for move in moves)
                summary_35, summary_4 = generate_summary(game.headers, moves_uci)
                if summary_35 and summary_4:
                    st.subheader("Game Summaries")
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write("**GPT-3.5 Turbo Summary**")
                        st.write(summary_35)
                    with col2:
